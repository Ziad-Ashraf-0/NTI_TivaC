...
void IntCtrl_Init(void)
{
    ...
    for (i = 0; i < INTCTRL_CONFIG_ARR_SIZE; i++)
    {
        ...
        Local_IntPriority =
            (IntCtrl_Config_Arr[i].IntCtrl_Group_Prio << 1) | (IntCtrl_Config_Arr[i].IntCtrl_SubGroup_Prio);
#elif (INTCTRL_GROUP_SUBGROUP_TYPE == INTCTRL_XYY)
        Local_IntPriority =
            (IntCtrl_Config_Arr[i].IntCtrl_Group_Prio << 2) | (IntCtrl_Config_Arr[i].IntCtrl_SubGroup_Prio);
